...
/--
Inverse operations for Ring, i.e. the additive inverse.
-/
class Ops (add_ident : outParam α) where
  add_inverse : (x : α) → α
export Ops (add_inverse)
...
export Props (
  ...
  m_substL m_substR m_assoc m_identL m_identR
)
...
attribute [implicit_reducible, instance] Ring.toOps
